...
describe('skill-generation', () => {
  describe('getSkillTemplates', () => {
    ...
    it('should include all expected skills', () => {
      ...
      expect(dirNames).toContain('infra-archive-change');
      expect(dirNames).toContain('infra-bulk-archive-change');
      expect(dirNames).toContain('infra-verify-change');
      expect(dirNames).toContain('infra-onboard');
      expect(dirNames).toContain('infra-propose');
    });
    ...
    it('should return single template when filter has one workflow', () => {
      ...
      expect(filtered[0].dirName).toBe('infra-propose');
    });
  });
  ...
  describe('generateSkillContent', () => {
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /infra:new to start and /infra:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/infra:/g, '/infra-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/infra-new');
      expect(content).toContain('/infra-apply');
      expect(content).not.toContain('/infra:new');
      expect(content).not.toContain('/infra:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /infra:new to start.',
      };
