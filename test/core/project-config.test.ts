  ...
  describe('readProjectConfig', () => {
    describe('resilient parsing', () => {
      it('should parse complete valid config', () => {
        const configDir = path.join(tempDir, 'infraspec');
...
`
      ...
      it('should handle completely invalid YAML gracefully', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Failed to parse infraspec/config.yaml'),
          expect.anything()
        );
      });
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit infraspec/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });
