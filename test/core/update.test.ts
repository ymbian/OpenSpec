  ...
  describe('basic validation', () => {
    it('should throw error if openspec directory does not exist', async () => {
      ...
      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No InfraSpec directory found. Run 'infraspec init' first."
      );
    });
...
`;
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'infra-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'infra-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('should update infra commands for configured Claude tool', async () => {
      ...
      // Check infra command files were created
    ...
    it('should update core profile infra commands when tool is configured', async () => {
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'infra-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'infra-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'infra-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'infra-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: infra-explore');
      ...
      expect(cursorSkill).toContain('name: infra-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'infra-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'infra-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: infra-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'infra-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'infra-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'infra-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'infra-apply-change', 'SKILL.md'),
        'old'
      );
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'infra-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'infra-explore', 'SKILL.md');
...
`;
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/infra:new')
      );
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'infra-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'infra-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'infra-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'infra-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'infra-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'infra-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New infra commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'infra');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'infra-propose', 'SKILL.md')
      )).toBe(false);
...
`
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'infra-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'infra-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'infraspec init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'infraspec init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'infra-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'infra-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'infra-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'infra-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'infra-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'infra-apply-change', 'SKILL.md'), 'content');
