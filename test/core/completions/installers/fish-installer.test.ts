...
describe('FishInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return standard fish completions path', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'infraspec.fish'));
    });
    ...
    it('should use homeDir from constructor', () => {
      ...
      expect(result).toBe(path.join(customHome, '.config', 'fish', 'completions', 'infraspec.fish'));
    });
  });
...
function __fish_infraspec
    echo "test"
end

complete -c infraspec -a 'init' -d 'Initialize OpenSpec'
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'infraspec.fish'));
    ...
    it('should create parent directories if they do not exist', async () => {
      ...
      expect(result.success).toBe(true);
      const targetPath = path.join(testHomeDir, '.config', 'fish', 'completions', 'infraspec.fish');
...
function __fish_infraspec_new
    echo "updated"
end

complete -c infraspec -a 'init' -d 'Initialize OpenSpec'
complete -c infraspec -a 'validate' -d 'Validate specs'
`;
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `infraspec fish test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('infraspec fish test');
