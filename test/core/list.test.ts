...
describe('ListCommand', () => {
  ...
  describe('execute', () => {
    it('should handle missing infraspec/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No InfraSpec changes directory found. Run 'infraspec init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'infraspec', 'changes');
