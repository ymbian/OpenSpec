...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  biz: {
    ...
    description: 'Create product-manager Word requirements',
  },
  wiki: {
    name: 'Code wiki',
...
function maybeWarnConfigDrift(
  ...
  console.log(colorize('Warning: Global config is not applied to this project. Run `infraspec update` to sync.'));
}
...
export function registerConfigCommand(program: Command): void {
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "infraspec config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: infraspec config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `infraspec config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
        ...
        console.log('Config updated. Run `infraspec update` in your projects to apply.');
        return;
      }
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `infraspec config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        if (fs.existsSync(openspecDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx infraspec update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `infraspec update` in your other projects to apply.');
            } catch {
              console.error('`infraspec update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
