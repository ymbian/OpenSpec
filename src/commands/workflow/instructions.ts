...
export async function generateApplyInstructions(
  ...
  const changeDir = path.join(projectRoot, 'infraspec', 'changes', changeName);
  ...
  if (missingArtifacts.length > 0) {
    state = 'blocked';
    instruction = `Cannot apply this change yet. Missing artifacts: ${missingArtifacts.join(', ')}.\nUse the infra-continue-change or infra-review-change skill to create the missing artifacts first.`;
  } else if (tracksFile && !tracksFileExists) {
    // Tracking file configured but doesn't exist yet
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `The ${tracksFilename} file is missing and must be created.\nUse infra-continue-change or infra-review-change to generate the tracking file.`;
  } else if (tracksFile && tracksFileExists && total === 0) {
    // Tracking file exists but contains no tasks
    const tracksFilename = path.basename(tracksFile);
    state = 'blocked';
    instruction = `The ${tracksFilename} file exists but contains no tasks.\nAdd tasks to ${tracksFilename} or regenerate it with infra-continue-change or infra-review-change.`;
...
export function printApplyInstructionsText(instructions: ApplyInstructions): void {
  ...
  if (state === 'blocked' && missingArtifacts) {
    ...
    console.log('Use the infra-continue-change or infra-review-change skill to create these first.');
    console.log();
  }
