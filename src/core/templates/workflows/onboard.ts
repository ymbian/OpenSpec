...
Explore mode (\`/infra:explore\`) is for this kind of thinking—investigating before implementing. You can use it anytime you need to think through a problem.
...
|---------|--------------|
| \`/infra:propose\` | Create a change and generate all artifacts |
| \`/infra:explore\` | Think through problems before/during work |
| \`/infra:apply\` | Implement tasks from a change |
...
| \`/infra:cicd\` | Trigger a remote CI/CD pipeline build and deploy |

---

## What's Next?

Try \`/infra:propose\` on something you actually want to build. You've got the rhythm now!
\`\`\`
...
| Command | What it does |
|---------|--------------|
| \`/infra:propose <name>\` | Create a change and generate all artifacts |
| \`/infra:explore\` | Think through problems (no code changes) |
| \`/infra:apply <name>\` | Implement tasks |
| \`/infra:archive <name>\` | Archive when done |
...
| \`/infra:cicd\` | Trigger a remote CI pipeline build |
