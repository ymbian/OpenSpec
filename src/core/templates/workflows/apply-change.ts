...
1. **Select the change**
   ...
   - If ambiguous, run \`infraspec list --json\` to get available changes and use the **AskUserQuestion tool** to let the user select

   Always announce: "Using change: <name>" and how to override (e.g., \`/infra:apply <other>\`).
...
3. **Get apply instructions**
   ...
   \`\`\`bash
   infraspec instructions apply --change "<name>" --json
   \`\`\`
   ...
   **Handle states:**
   - If \`state: "blocked"\` (missing artifacts): show message, suggest using infra-review-change
...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If \`state: "blocked"\` (missing artifacts): show message, suggest using \`/infra:review\`
   - If \`state: "all_done"\`: congratulate, suggest archive
   - Otherwise: proceed to implementation
...
All tasks complete! You can archive this change with \`/infra:archive\`.
\`\`\`
