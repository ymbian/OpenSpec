...
# Use infraspec __complete to get available changes
_infraspec_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(infraspec __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use infraspec __complete to get available specs
_infraspec_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(infraspec __complete specs 2>/dev/null)
