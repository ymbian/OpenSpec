...
/**
 * Generates Bash completion scripts for the InfraSpec CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for InfraSpec CLI
...
${helpers}
complete -F _infraspec_completion infraspec
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_infraspec_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_infraspec_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_infraspec_complete_items`);
        break;
      case 'shell':
