...
/**
 * Read and parse infraspec/config.yaml from project root.
 ...
 *
 * @param projectRoot - The root directory of the project (where `infraspec/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  let configPath = path.join(projectRoot, 'infraspec', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'infraspec', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`infraspec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse infraspec/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in infraspec/config.yaml\n\n`;
  ...
  message += `\nFix: Edit infraspec/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
