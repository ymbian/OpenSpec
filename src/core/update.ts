...
export class UpdateCommand {
  ...
  async execute(projectPath: string): Promise<void> {
    ...
    // Use detected tool directories to preserve existing infra skills/commands.
    ...
    if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
      ...
      console.log(chalk.dim('Run "infraspec init" to set up tools.'));
      return;
    }
  ...
  private detectNewTools(projectPath: string, configuredTools: string[]): void {
    ...
    if (newTools.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'infraspec init' to add ${pronoun}.`
        )
      );
    }
  }
  ...
  private displayExtraWorkflowsNote(
    ...
    if (extraWorkflows.length > 0) {
      console.log(chalk.dim(`Note: ${extraWorkflows.length} extra workflows not in profile (use \`infraspec config profile\` to manage)`));
    }
  }
