...
function scoreSymbol(symbol: CodeSymbol, file: CodeFile | undefined, terms: string[], query: string): ScoredSymbol | null {
  ...
  if ((symbol.kind === 'function' || symbol.kind === 'method' || symbol.kind === 'class') && score > 0) {
    score += 8;
  }
...
export async function buildCodeContext(
  ...
  const blockCandidates = [...entrySymbols, ...relatedSymbols]
    .filter((item) => ['function', 'method', 'class'].includes(item.symbol.kind));
  ...
  if (index.stats.skippedLargeFiles > 0) {
    gaps.push(`${index.stats.skippedLargeFiles} large files were skipped during lightweight indexing.`);
  }
...
export function formatCodeContextMarkdown(context: CodeContext, requirementPath: string, indexPath: string): string {
  const lines: string[] = [
    ...
    '',
    'This file is generated by `infraspec code analyze` using InfraSpec\'s built-in lightweight code graph.',
    '',
    '## Summary',
    '',
  ...
  if (context.relatedSymbols.length === 0) {
    lines.push('- No related symbols found from lightweight graph expansion.');
  } else {
