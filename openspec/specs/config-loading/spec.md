...
## Purpose
Define how `infraspec/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from infraspec/config.yaml

The system SHALL read and parse the project configuration file located at `infraspec/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `infraspec/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `infraspec/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `infraspec/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `infraspec/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `infraspec/config.yml` exists and `infraspec/config.yaml` does not exist
- **THEN** system reads from `infraspec/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `infraspec/config.yaml` and `infraspec/config.yml` exist
- **THEN** system prefers `infraspec/config.yaml`
